...
// node_modules/@orama/wc-components/dist/esm/utils-dee0615d.js
var STEMMERS = {
  arabic: "ar",
  armenian: "am",
  bulgarian: "bg",
...
function getNanosecondsTime() {
  ...
  if (typeof process !== "undefined" && typeof ((_a = process == null ? void 0 : process.hrtime) == null ? void 0 : _a.bigint) === "function") {
    return process.hrtime.bigint();
  }
...
function isAsyncFunction(func) {
  var _a;
  return ((_a = func == null ? void 0 : func.constructor) == null ? void 0 : _a.name) === "AsyncFunction";
}
...
function calculateNumberFacetBuilder(ranges, values, alreadyInsertedValues) {
  return (facetValue) => {
    for (const range of ranges) {
      ...
      if (alreadyInsertedValues == null ? void 0 : alreadyInsertedValues.has(value)) {
        continue;
      }
      if (facetValue >= range.from && facetValue <= range.to) {
        if (values[value] === void 0) {
          ...
          values[value]++;
          alreadyInsertedValues == null ? void 0 : alreadyInsertedValues.add(value);
        }
      }
    }
  };
...
function calculateBooleanStringOrEnumFacetBuilder(values, propertyType, alreadyInsertedValues) {
  ...
  return (facetValue) => {
    const value = (facetValue == null ? void 0 : facetValue.toString()) ?? defaultValue;
    if (alreadyInsertedValues == null ? void 0 : alreadyInsertedValues.has(value)) {
      return;
    }
...
function fullTextSearch(orama, params, language) {
  ...
  const asyncNeeded = ((_a = orama.beforeSearch) == null ? void 0 : _a.length) || ((_b = orama.afterSearch) == null ? void 0 : _b.length);
  if (asyncNeeded) {
...
function innerVectorSearch(orama, params, language) {
  ...
  if ((vector == null ? void 0 : vector.value.length) !== vectorSize) {
    if ((vector == null ? void 0 : vector.property) === void 0 || (vector == null ? void 0 : vector.value.length) === void 0) {
...
var AnswerSession = class {
  ...
  regenerateLast({ stream = true }) {
    ...
    const isLastMessageAssistant = ((_a = this.messages.at(-1)) == null ? void 0 : _a.role) === "assistant";
  ...
  async init() {
    ...
    if ((_b = (_a = pluginExtras == null ? void 0 : pluginExtras.pluginParams) == null ? void 0 : _a.chat) == null ? void 0 : _b.model) {
...
var Switch = class {
  constructor(client) {
    __publicField(this, "invalidClientError", "Invalid client. Expected either an OramaClient or an Orama OSS database.");
    __publicField(this, "client");
    __publicField(this, "clientType");
    __publicField(this, "isCloud", false);
    __publicField(this, "isOSS", false);
    this.client = client;
  ...
  createAnswerSession(params) {
    ...
    if (this.isOSS) {
      const p = params;
      return new AnswerSession(this.client, {
...
function arrowKeysNavigation(ref, event, selector = "[focus-on-arrow-nav]") {
  ...
  const focusedElement = ref.querySelector(":focus");
  const focusedIndex = focusableArray.indexOf(focusedElement);
  let nextFocusableElement;
  if (event.key === "ArrowDown") {
    nextFocusableElement = focusedIndex === focusableArray.length - 1 ? firstFocusableElement : focusableArray[focusedIndex + 1];
...
function getNonExplicitAttributes(element, explicitProps) {
...
function validateCloudIndexConfig(el, index, instance) {
  const componentDetails = `
    Component: ${el.tagName.toLowerCase()}
    Id: ${el.id}
  `;
  if (!index && !instance) {
    throw new Error(`Invalid component configuration. Please provide a valid index or instance prop. ${componentDetails}`);
  }
  if (instance && !index) {
    const oramaInstance = new Switch(instance);
    oramaInstance.search({
      term: "test"
    }).catch(() => {
      throw new Error(`Invalid cloud instance configuration. Please provide a valid api_key and endpoint ${componentDetails}`);
    });
  }
  if (index && !instance) {
    if (!index.api_key || !index.endpoint) {
      throw new Error(`Invalid cloud index configuration. Please provide a valid api_key and endpoint ${componentDetails}`);
    }
    return;
  }
  if (index && instance) {
...
function initOramaClient(index) {
  return new It({
    api_key: index.api_key,
    endpoint: index.endpoint
  });
}
function generateRandomID(componentName) {
...
@orama/wc-components/dist/esm/utils-dee0615d.js:
  (*! Bundled license information:
  ...
  *)
*/
//# sourceMappingURL=chunk-HXUFFIHY.js.map
